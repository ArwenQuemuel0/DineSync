...
const APP_TIME_ZONE = process.env.EXPO_PUBLIC_APP_TIMEZONE || "Asia/Manila";
...
export default function OrderStatusScreen({ route, navigation }) {
  ...
  const responsive = useMemo(() => {
    ...
    const isVeryNarrow = usableWidth < 390;
    ...
    const scale = (size, min = size * 0.75, max = size * 1.12) => {
    ...
    return {
      ...
      containerPadding: isVeryNarrow
        ? scale(12, 10, 14)
        : isPhone
          ? scale(14, 12, 16)
          : scale(24, 16, 26),

      topBarHeight: useCompactHeader
        ? scale(50, 44, 54)
        : isPhone
          ? scale(56, 48, 60)
          : scale(66, 52, 70),

      backText: useCompactHeader
        ? scale(13, 11, 14)
        : isPhone
          ? scale(16, 14, 17)
          : scale(24, 16, 24),

      tableText: useCompactHeader
        ? scale(13, 11, 14)
        : isPhone
          ? scale(16, 14, 17)
          : scale(24, 16, 24),
      ...
      subHeader: isPhone ? scale(17, 15, 19) : scale(26, 18, 26),

      subHeaderMargin: isPhone ? scale(12, 10, 14) : scale(18, 14, 20),

      loadingText: isPhone ? scale(18, 15, 20) : scale(22, 16, 22),

      errorText: isPhone ? scale(14, 12, 15) : scale(16, 12, 16),

      cardPadding: isVeryNarrow
        ? scale(14, 12, 16)
        : isPhone
          ? scale(16, 14, 18)
          : scale(22, 16, 24),

      cardRadius: scale(22, 16, 24),

      cardMargin: isPhone ? scale(16, 14, 18) : scale(18, 14, 20),

      orderTitle: isPhone ? scale(19, 17, 21) : scale(26, 19, 26),

      orderMeta: isPhone ? scale(13, 11, 14) : scale(15, 11, 15),

      statusText: isPhone ? scale(13, 11, 14) : scale(16, 11, 16),

      paymentBadgeText: isPhone ? scale(12, 10, 13) : scale(14, 11, 14),

      badgePaddingV: isPhone ? scale(7, 5, 8) : scale(8, 5, 8),

      badgePaddingH: isPhone ? scale(11, 9, 13) : scale(18, 10, 18),

      paymentMessage: isPhone ? scale(14, 12, 15) : scale(16, 12, 16),

      paymentMessageLine: isPhone ? scale(20, 17, 21) : scale(22, 17, 22),

      dividerMargin: isPhone ? scale(12, 10, 14) : scale(16, 10, 16),

      noItemsText: isPhone ? scale(14, 12, 15) : scale(16, 12, 16),

      itemName: isPhone ? scale(15, 13, 17) : scale(19, 14, 19),

      itemQty: isPhone ? scale(13, 11, 14) : scale(15, 11, 15),

      customPrice: isPhone ? scale(13, 11, 14) : scale(14, 11, 14),

      requestText: isPhone ? scale(13, 11, 14) : scale(14, 11, 14),

      itemPrice: isPhone ? scale(15, 13, 17) : scale(18, 13, 18),

      totalLabel: isPhone ? scale(17, 15, 19) : scale(22, 16, 22),

      totalValue: isPhone ? scale(21, 18, 23) : scale(26, 20, 26),

      emptyIcon: isPhone ? scale(66, 52, 72) : scale(80, 55, 82),

      emptyTitle: isPhone ? scale(23, 20, 25) : scale(36, 24, 36),

      emptyText: isPhone ? scale(15, 13, 17) : scale(18, 13, 18),

      emptyLine: isPhone ? scale(22, 19, 24) : scale(25, 19, 25),

      footerPadding: isPhone ? scale(10, 8, 12) : scale(14, 10, 14),

      footerRadius: scale(18, 14, 18),

      autoText: isPhone ? scale(13, 11, 14) : scale(15, 11, 15),

      buttonPaddingV: isPhone ? scale(10, 8, 11) : scale(11, 8, 11),

      buttonPaddingH: isPhone ? scale(14, 10, 16) : scale(22, 14, 22),

      buttonRadius: scale(12, 9, 12),

      buttonText: isPhone ? scale(15, 13, 16) : scale(16, 12, 16),

      listBottom: scale(14, 10, 18),

      maxContentWidth: isLandscape
        ? clamp(usableWidth * 0.92, 360, 980)
        : clamp(usableWidth * 0.96, 320, 760),
    };
  }, [width, height, insets.left, insets.right, insets.bottom]);

  const { tableNumber, user } = useAuth();

  const { tableResetRequired, acknowledgeTableReset } = useTableStatus();
  ...
  useEffect(() => {
    const loadCompletedQrPaymentProcesses = async () => {
      try {
        ...
        console.log("LOAD COMPLETED QR PAYMENT PROCESS ERROR:", storageError);
      }
    };
...
const isVisibleOrderStatus = (order) => {
  const normalized = normalizeOrderStatus(order?.status);

  if (normalized === "served" || normalized === "completed") {
    const updatedAt = order?.updated_at || order?.served_at;

    if (!updatedAt) {
      return false;
    }

    const servedTime = new Date(updatedAt).getTime();
    const now = Date.now();

    return now - servedTime <= 5000;
  }

  return isActiveOrderStatus(normalized) || normalized === "awaiting_payment";
};
  ...
  const fetchOrders = async (showLoading = true) => {
    try {
      ...
      if (!finalTableNumber) {
        setError("No table number found.");

        return;
      }

      if (orderId) {
        const specificOrder = await fetchSpecificOrder();

      if (specificOrder) {
  console.log("ORDER STATUS FROM BACKEND:", specificOrder.status);
  console.log("PAYMENT STATUS FROM BACKEND:", specificOrder.payment_status);

  if (isVisibleOrderStatus(specificOrder)) {
  ...
  return;
      ...
      const response = await getActiveTableOrders(finalTableNumber);

      if (response.success) {
        const activeOrders = (response.data || []).filter((order) =>
  isVisibleOrderStatus(order),
);
  ...
  const normalizePaymentMethod = (value) => {
    const normalized = String(value || "Pay Later")
      .trim()
      .toLowerCase()
      .replace(/[_-]+/g, " ")
      .replace(/\s+/g, " ");
    ...
    if (normalized === "cash" || normalized === "cash paid") {
      return "Cash";
    }

    if (normalized === "pay later" || normalized === "later") {
      return "Pay Later";
    }
  ...
  const getOrderInvoiceUrl = (order) => {
    return (
  ...
  const isSameRouteOrder = (order) => {
    return orderId && Number(order?.id) === Number(orderId);
  };

  const hasCompletedQrPaymentProcess = (order) => {
    ...
    return (
      completedQrPaymentProcessOrderIds.includes(orderIdString) ||
      (qrPaymentProcessCompleted && isSameRouteOrder(order))
    );
  };

  const canContinueQrPayment = (order) => {
    const paymentMethod = normalizePaymentMethod(order.payment_method);
    ...
    const completedProcess = hasCompletedQrPaymentProcess(order);
  ...
  const handleContinueQrPayment = (order) => {
    const invoiceUrl = getOrderInvoiceUrl(order);
  ...
  const isCustomOrderItem = (orderItem) => {
    const category = String(
      orderItem?.category || orderItem?.menu_item?.category || "",
    )
      .trim()
      .toLowerCase();

    const inventoryType = String(
      orderItem?.inventory_type || orderItem?.menu_item?.inventory_type || "",
    )
      .trim()
      .toLowerCase();
  ...
  const getRequestText = (orderItem) => {
    return (
  ...
  const getDisplayOrderStatusLabel = (order) => {
    const normalized = getEffectiveOrderStatus(order);
...
if (isPaidStatus && orderStatus === "pending") {
  ...
  const getOrderTotal = (order) => {
    if (order.total_amount || order.total) {
      return Number(order.total_amount || order.total);
    }
  ...
  const renderOrderItem = ({ item }) => {
    ...
    const paymentMessage = getPaymentMessage(item);

    const paymentMethod = getPaymentMethodLabel(item);

    const showContinueQrPayment = canContinueQrPayment(item);
      ...
      >
          ...
          <View style={styles.orderHeaderLeft}>
            <Text
              style={[
                styles.orderTitle,
                {
                  fontSize: responsive.orderTitle,
                },
              ]}
              numberOfLines={2}
            >
              {item.order_number ? item.order_number : `Order #${item.id}`}
            </Text>
            ...
            >
              <Text
                style={[
                  ...
                  {
                    fontSize: responsive.statusText,
                  },
                ]}
                numberOfLines={1}
            ...
            >
              <Text
                style={[
                  ...
                  {
                    fontSize: responsive.paymentBadgeText,
                  },
                ]}
                numberOfLines={1}
                adjustsFontSizeToFit
        ...
        {paymentMessage ? (
          <Text
            style={[
              ...
              {
                fontSize: responsive.paymentMessage,
                lineHeight: responsive.paymentMessageLine,
              },
            ]}
          >
            {paymentMessage}
        ...
        {showContinueQrPayment ? (
          <TouchableOpacity
            ...
            onPress={() => handleContinueQrPayment(item)}
          >
            <Text
              style={[
        ...
        <View
          style={[
            styles.divider,
            {
              marginVertical: responsive.dividerMargin,
            },
          ]}
        />
        ...
        {items.length === 0 ? (
          <Text
            style={[
              styles.noItemsText,
              {
                fontSize: responsive.noItemsText,
              },
            ]}
          >
          ...
          items.map((orderItem, index) => {
            ...
            const price = customItem
              ? 0
              : Number(orderItem.price || orderItem.menu_item?.price || 0);

            const requestText = getRequestText(orderItem);
                    ...
                    <>
                      <Text
                        style={[
                          ...
                          {
                            fontSize: responsive.customPrice,
                          },
                        ]}
                      >
      ...
      <View style={styles.frame}>
        ...
        >
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color="#f68c45" />

            <Text
              style={[
                styles.loadingText,
                {
                  fontSize: responsive.loadingText,
                },
              ]}
            >
        ...
        <View
          style={[
            styles.container,
            {
              ...
              paddingBottom: responsive.safeBottomExtra,
            },
          ]}
        >
          <View
          ...
          <Text
            style={[
              styles.subHeader,
              {
                fontSize: responsive.subHeader,
                marginBottom: responsive.subHeaderMargin,
              },
            ]}
            numberOfLines={1}
            adjustsFontSizeToFit
            minimumFontScale={0.75}
          >
            {orderId ? `Order #${orderId}` : "Active Orders"}
          </Text>
            ...
            <View style={styles.emptyBox}>
              ...
              <Text
                style={[
                  styles.emptyTitle,
                  {
                    fontSize: responsive.emptyTitle,
                  },
                ]}
              >
                No Active Orders
              ...
              <Text
                style={[
                  styles.emptyText,
                  {
                    fontSize: responsive.emptyText,
                    lineHeight: responsive.emptyLine,
                  },
                ]}
              >
                Pending, preparing, ready, and awaiting payment orders will
                appear here based on kitchen progress.
              </Text>
            </View>
          ) : (
            <FlatList
              ...
              contentContainerStyle={[
                styles.listContent,
                {
                  paddingBottom: responsive.listBottom,
            ...
            <View
              style={[
                styles.footerCard,
                {
                  ...
                  borderRadius: responsive.footerRadius,
                  flexDirection: responsive.bottomBarNeedsWrap
                    ? "column"
                    : "row",
                  alignItems: responsive.bottomBarNeedsWrap
                    ? "stretch"
                    : "center",
                },
              ]}
              ...
              <View style={styles.autoInfo}>
                ...
                <Text
                  style={[
                    styles.autoText,
                    {
                      fontSize: responsive.autoText,
                      textAlign: responsive.bottomBarNeedsWrap
                        ? "center"
                        : "left",
                    },
                  ]}
